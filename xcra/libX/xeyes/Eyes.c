...
static void eyeLiner __P((EyesWidget, Drawable, GC, GC, int));
static void eyeBall __P((EyesWidget, GC, int));
    ...
    Widget greq, gnew;
{
    ...
    w->eyes.shape_mask = 0;
    w->eyes.shapeGC = 0;
    w->eyes.m_dx = INVALID_POS;
    w->eyes.m_dy = INVALID_POS;
}
...
static void computePupil (ret, num, mouse)
    TPoint*	ret;
    int		num;
    TPoint	mouse;
{
...
#else
		/*
		 * New formula after mathematical transformations
		 */
#define sq(X)	((X)*(X))
#define	A	(sq(EYE_HHEIGHT) * sq(BALL_DIST))
#define B	(((sq(EYE_HHEIGHT) / sq(EYE_HWIDTH))) - 1.0)

		dist = A / (1.0 + B * (cosa * cosa));
		dx = sq(dx);
		dy = sq(dy);
		dx += dy;
		if (dist > dx) {
...
static void computePupils (mouse, pupils)
    ...
    TPoint	pupils[2];
{
    computePupil (&pupils[0], 0, mouse);
    computePupil (&pupils[1], 1, mouse);
}

static void recomputePupils(w, dx, dy)
EyesWidget	w;
int		dx;
int		dy;
{
     ...
     XtIntervalId *id;		/* unused */
{
	...
	if (dx != w->eyes.m_dx || dy != w->eyes.m_dy) {
	    w->eyes.m_dx = dx;
	    w->eyes.m_dy = dy;

	    recomputePupils(w, dx, dy);
	} else {
	    if (delays[w->eyes.update + 1] != 0)
	      ++w->eyes.update;
	}
